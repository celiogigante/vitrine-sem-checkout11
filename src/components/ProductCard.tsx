...
import { Product, conditionLabel, conditionColor, getWhatsAppLink, statusLabel, statusColor } from "@/lib/products";
...
const ProductCard = ({ product }: { product: Product }) => {
  ...
  return (
    <div className={`group rounded-xl border bg-card overflow-hidden transition-shadow hover:shadow-lg ${sold ? "opacity-60" : ""}`} style={{ boxShadow: "var(--card-shadow)" }}>
        ...
        <div className="flex gap-2">
          ...
          {!sold && (
            <Button asChild size="sm" className="bg-whatsapp hover:bg-whatsapp-hover text-whatsapp-foreground">
              <a href={getWhatsAppLink(product)} target="_blank" rel="noopener noreferrer" aria-label="Negociar via WhatsApp">
